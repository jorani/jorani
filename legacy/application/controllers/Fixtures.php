...
class Fixtures extends CI_Controller
{
    ...
    public function generate(): void
    {
        ...
        $this->db->truncate('dayoffs');
        $this->db->truncate('delegations');
        $this->db->truncate('entitleddays');
        $this->db->truncate('leaves');
        $this->db->truncate('oauth_clients');
        $this->db->truncate('organization');
        $this->db->truncate('overtime');
        ...
        $orgs = [
            ...
            ['id' => 1, 'name' => 'IT Department', 'parent_id' => 0, 'supervisor' => null],
            ['id' => 2, 'name' => 'HR Department', 'parent_id' => 0, 'supervisor' => null],
            ['id' => 3, 'name' => 'Staff', 'parent_id' => 0, 'supervisor' => 1],
            ['id' => 4, 'name' => 'Management', 'parent_id' => 3, 'supervisor' => 1],
            ['id' => 5, 'name' => 'Employees', 'parent_id' => 3, 'supervisor' => 3],
            ['id' => 6, 'name' => 'Branch 1', 'parent_id' => 3, 'supervisor' => 4],
            ['id' => 7, 'name' => 'Dept 1', 'parent_id' => 6, 'supervisor' => 4],
            ['id' => 8, 'name' => 'Dept 2', 'parent_id' => 6, 'supervisor' => 4],
            ...
            ['id' => 12, 'name' => 'Branch 3', 'parent_id' => 3, 'supervisor' => 2],
            ['id' => 13, 'name' => 'Dept 1', 'parent_id' => 12, 'supervisor' => 2],
            ['id' => 14, 'name' => 'Dept 2', 'parent_id' => 12, 'supervisor' => 2],
        ];
        ...
        // Users
        $users = [
            ['id' => 1, 'firstname' => 'Admin', 'lastname' => 'ADMINISTRATOR', 'login' => 'jorani', 'email' => 'jorani@example.org', 'password' => '$2a$08$My0FoP4zalOiS2OM6rsCz.XXozSW1UiqRq4zKup/CC/kOxXDCs9Zm', 'role' => 9, 'manager' => 1, 'organization' => 0, 'contract' => 1, 'position' => 1, 'language' => 'en', 'active' => 1, 'timezone' => 'Europe/Paris'],
            ['id' => 2, 'firstname' => 'John', 'lastname' => 'DOE', 'login' => 'jdoe', 'email' => 'jdoe@example.org', 'password' => '$2a$08$TU9Syb0FTqcKGwAg5AtH.OLThxcR3p.1gKEcorxvbuqKWTyHQRI7y', 'role' => 1, 'manager' => 1, 'organization' => 0, 'contract' => 1, 'position' => 2, 'language' => 'en', 'active' => 1, 'timezone' => 'Europe/Paris'],
            ['id' => 3, 'firstname' => 'Alice', 'lastname' => 'DOE', 'login' => 'alice', 'email' => 'alice@example.org', 'password' => '$2a$08$lzQzTsCKUECKNP6h.r25BOvJStGTzqB5Ljp9KBzXeLiZaYZtjZuxm', 'role' => 1, 'manager' => 1, 'organization' => 0, 'contract' => 1, 'position' => 3, 'language' => 'en', 'active' => 1, 'timezone' => 'Europe/Paris'],
            ['id' => 4, 'firstname' => 'Bob', 'lastname' => 'Dylan', 'login' => 'bob', 'email' => 'bob@example.org', 'password' => '$2a$08$LwiKZL/pa7UhRjGDCtMpHOU8pQc2Zrkje2cUBRJ1W.tX/SuE40n7a', 'role' => 1, 'manager' => 4, 'organization' => 0, 'contract' => 1, 'position' => 3, 'language' => 'en', 'active' => 1, 'timezone' => 'Europe/Paris']
        ];
        for ($ii = 0; $ii < 100; $ii++) {
            ...
            $users[] = [
                ...
                'role' => 2,
                'manager' => $this->faker->numberBetween(1, 4),
        ...
        echo "Users inserted. (Passwords: jorani for jorani (admin), alice for alice, bob for bob, password for others)\n";

        // Delegations
        $delegations = [
            ['id' => 1, 'manager_id' => 2, 'delegate_id' => 4],
        ];
        $this->db->insert_batch('delegations', $delegations);
        echo "Delegations inserted. Bob has no line manager and has delegation from John Doe\n";

        // Entitled Days (Balances)
        $entitledDays = [];
        ...
        for ($ii = 0; $ii < 1000; $ii++) {
            ...
            // Maximum end date allowed by business rule
            $maxEndDateBySpan = $startDate->modify('+5 days');

            // Last day of the start year
            $endOfYear = $startDate->setDate(
                (int) $startDate->format('Y'),
                12,
                31
            );

            // Keep the end date within the same year
            if ($maxEndDateBySpan > $endOfYear) {
                $endDate = $endOfYear;
            } else {
                $endDate = $maxEndDateBySpan;
            }
            ...
            $leaves[] = [
                ...
                'employee' => $this->faker->numberBetween(1, 100),
        ...
        echo "Leaves inserted (1000 leaves). Except for user id > 100.\n";
        ...
        echo "Overtime requests inserted (100 overtimes). Except for user id > 100.\n";
        ...
        // We need this in order to test the API
        $oauth_clients = [
            ['client_id' => 'testclient', 'client_secret' => 'testpass', 'redirect_uri' => 'http://fake/']
        ];
        $this->db->insert_batch('oauth_clients', $oauth_clients);
        echo "OAuth clients inserted. (testclient, testpass, http://fake/)\n";

...
if ($mod_rewrite == "")
    $mod_rewrite = '<b>.htaccess not visited</b>';
if ($allow_overwrite == "")
    ...
    <div class="container">
        ...
        <table class="table table-bordered table-hover table-condensed">
            ...
            <tbody id="tblServer">
                <?php if ($env != '') { ?>
                    <tr>
                        <td><i class="mdi mdi-information-outline"></i>&nbsp;Environment</td>
                        <td><?php echo $env; ?></td>
                    </tr>
                <?php } ?>

                <tr>
                    <td><i class="mdi mdi-information-outline"></i>&nbsp;Server software</td>
                    <td><?php echo $server_software; ?></td>
                </tr>

                <tr>
                    <td><?php if (strtolower($allow_overwrite) == "on") { ?><i
                                class="mdi mdi-check"></i><?php } else { ?><i class="mdi mdi-alert"></i><?php } ?>
                        &nbsp;Allow overwrite (.htaccess files)</td>
                    <td><?php echo $allow_overwrite; ?> (used for cool URLs). Ignore this message if you are running
                        something else than Apache.</td>
                </tr>

                <tr>
                    <td><?php if ($mod_rewrite) { ?><i class="mdi mdi-check"></i><?php } else { ?><i
                                class="mdi mdi-alert"></i><?php } ?>
                        &nbsp;Apache module rewrite (mod_rewrite)</td>
                    <td><?php echo $mod_rewrite; ?> (used for cool URLs). Ignore this message if you are running
                        something else than Apache.</td>
                </tr>
                ...
                <?php if (function_exists('mb_strimwidth')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;<code>mb_strimwidth</code> function exists</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;<code>mb_strimwidth</code> function doesn't exist</td>
                    <?php } ?>
                    <td>PHP must be compiled with <a href="http://php.net/manual/en/mbstring.installation.php"
                            target="_blank">multibyte string support<a>.</td>
                </tr>

                <?php if (function_exists('json_encode')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;<code>json_encode</code> function exists</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;<code>json_encode</code> function doesn't exist</td>
                    <?php } ?>
                    <td>PHP must be compiled with <a href="http://php.net/manual/en/json.installation.php"
                ...
                <?php if (extension_loaded('pdo_mysql')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;pdo_mysql is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;PDO/mysql IS NOT LOADED.</td>
                    <?php } ?>
                    <td>PDO/mysql is the recommended database driver.</td>
                </tr>

                <?php if (extension_loaded('Zend OPcache')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;OPcache is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;OPcache IS NOT LOADED.</td>
                    <?php } ?>
                    <td>Please consider activating OPcache for the best performances.</td>
                </tr>

                <?php if (extension_loaded('openssl')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;openssl is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;openssl IS NOT LOADED.</td>
                    <?php } ?>
                    <td>PHP Extension openssl is required if you use PHP7.1.</td>
                </tr>

                <?php if (extension_loaded('curl')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;curl is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;curl IS NOT LOADED.</td>
                    <?php } ?>
                    <td>PHP Extension curl is needed for OAuth2 authentication.</td>
                </tr>

                <?php if (extension_loaded('ldap')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;ldap is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;ldap IS NOT LOADED</td>
                    <?php } ?>
                    <td>PHP Extension ldap is optional and allows you to use LDAP for authentication.</td>
                </tr>

                <?php if (extension_loaded('zip')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;zip is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;zip IS NOT LOADED</td>
                    <?php } ?>
                    <td>PHP Extension zip allows you to use the export to Excel feature.</td>
                </tr>

                <?php if (extension_loaded('xml')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;xml is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;xml IS NOT LOADED</td>
                    <?php } ?>
                    <td>PHP Extension xml allows you to use the export to Excel feature (and SAML/SSO).</td>
                </tr>

                <?php if (extension_loaded('gd')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;gd is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;gd IS NOT LOADED</td>
                    <?php } ?>
                    <td>PHP Extension gd2 allows you to use the export to Excel feature.</td>
                </tr>

                <?php if (extension_loaded('date')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;date is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;date IS NOT LOADED</td>
                    <?php } ?>
                    <td>PHP Extension zlib allows you to use the Authentication by SAML feature (SSO).</td>
                </tr>

                <?php if (extension_loaded('zlib')) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;zlib is LOADED</td>
                    <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;zlib IS NOT LOADED</td>
                    <?php } ?>
                    <td>PHP Extension zlib allows you to use the Authentication by SAML feature (SSO).</td>
                </tr>
        ...
        <table class="table table-bordered table-hover table-condensed">
            ...
            <tbody id="tblDatabase">
                <?php if ($configFileExists) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;Configuration file</td>
                        <td>Found</td>
                    </tr>
                <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;Configuration file</td>
                        <td>Not Found</td>
                    </tr>
                <?php } ?>
                ...
                <?php if (!$dbConnError) { ?>
                    <tr>
                        <td><i class="mdi mdi-check"></i>&nbsp;Database connection</td>
                        <td>OK</td>
                    </tr>
                <?php } else { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;Database connection</td>
                        <td>Error</td>
                    </tr>
                <?php } ?>
                ...
                <?php if (is_null($rowOrg)) { ?>
                    <tr>
                        <td><i class="mdi mdi-alert"></i>&nbsp;Organization structure</td>
                        <td>No root entity was found.</td>
                    </tr>
                <?php } else { ?>
                    <?php if ($rowOrg['id'] != 0) { ?>
                        <tr>
                            <td><i class="mdi mdi-alert"></i>&nbsp;Organization structure</td>
                            <td>The root entity must be equal to zero. To fix a problem of backup/restore, please execute this
                                query: <br />
                                <code>UPDATE `organization` SET `organization`.`id` = 0 WHERE `parent_id` = -1</code>
                            </td>
                        </tr>
                    <?php } else { ?>
                        <tr>
                            <td><i class="mdi mdi-check"></i>&nbsp;Organization structure</td>
                            <td>OK</td>
                        </tr>
                    <?php }
                } ?>
            ...
            <tbody id="tblSchema">
                <?php if (!$dbConnError && !$dbQueryError) {
                    foreach ($rowsSchema as $row) { ?>
                        <tr>
                            <td><i class="mdi mdi-information-outline"></i>&nbsp;<?php echo $row['TABLE_NAME']; ?></td>
                            <td><?php echo $row['signature']; ?></td>
                        </tr>
                    <?php }
                } else { ?>
                    <tr>
                        <td colspan="2"><i>Impossible to query database</i></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
